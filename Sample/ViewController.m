...
static const NSInteger kLitterSize = 100;            // intial number of kitten cells in ASTableView
static const NSInteger kLitterBatchSize = 100;       // number of kitten cells to add to ASTableView
static const NSInteger kMaxLitterSize = 1000;        // max number of kitten cells allowed in ASTableView
...
- (instancetype)init
{
  ...
  tuningParameters.leadingBufferScreenfuls = 1.f;
  tuningParameters.trailingBufferScreenfuls = 1.f;
  [_tableView setTuningParameters:tuningParameters
                     forRangeType:ASLayoutRangeTypePreload];
  [_tableView setTuningParameters:tuningParameters
                     forRangeType:ASLayoutRangeTypeRender];
...
//                                   target:self
//                                 selector:@selector(createImageViews)
//                                 userInfo:nil
//                                  repeats:YES];
//  [NSTimer scheduledTimerWithTimeInterval:5.f
//                                   target:_tableView
...
- (void)createImageViews
{
  ...
  for (int i = 0; i < 50; i++)
  {
    [self.view addSubview:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icn_attachement"]]];
    [self.view addSubview:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"circle_gray_filled"]]];
    [self.view addSubview:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icn_checkbox_grid_select"]]];
    [self.view addSubview:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"circle_gray_filled"]]];
  }
}
