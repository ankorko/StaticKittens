...
#define MaxImageNodeIterations 10
...
- (instancetype)initWithKittenOfSize:(CGSize)size
{
  ...
  for (int i = 0; i < MaxImageNodeIterations; i++) {
    ASImageNode *imageNode = [[ASImageNode alloc] init];
    imageNode.image = [UIImage imageNamed:@"circle_gray_filled"];
    [self addSubnode:imageNode];
  }
  for (int i = 0; i < MaxImageNodeIterations; i++) {
    ASImageNode *imageNode = [[ASImageNode alloc] init];
    imageNode.image = [UIImage imageNamed:@"selection_circle"];
    [self addSubnode:imageNode];
  }
  for (int i = 0; i < MaxImageNodeIterations; i++) {
    ASImageNode *imageNode = [[ASImageNode alloc] init];
    imageNode.image = [UIImage imageNamed:@"icn_checkbox_grid_select"];
    [self addSubnode:imageNode];
  }
  for (int i = 0; i < MaxImageNodeIterations; i++) {
    ASImageNode *imageNode = [[ASImageNode alloc] init];
    imageNode.image = [UIImage imageNamed:@"badge"];
    [self addSubnode:imageNode];
  }
  for (int i = 0; i < MaxImageNodeIterations; i++) {
    ASImageNode *imageNode = [[ASImageNode alloc] init];
    imageNode.image = [UIImage imageNamed:@"icn_attachement"];
    [self addSubnode:imageNode];
  }
  ...
  _imageNode.imageModificationBlock = ASImageNodeRoundBorderModificationBlock(0.f, [UIColor clearColor]);
...
- (void)layout
{
  ...
  {
    if ([node isMemberOfClass:[ASImageNode class]])
    {
      ASImageNode *imageNode = (ASImageNode *)node;
      imageNode.frame = CGRectMake(imageX, 0.f, 40.f, 40.f);
      imageX += 45.f;
      imageX =  remainderf(imageX, 700.f);
    }
  }
